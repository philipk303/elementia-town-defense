...
class WaterPreviewScene extends Phaser.Scene {
  ...
  preload() {
    this.load.atlas(WATER_HERO_ATLAS, WATER_HERO_PATHS.png, WATER_HERO_PATHS.json)
    this.load.atlas(WATER_FX_ATLAS, WATER_FX_PATHS.png, WATER_FX_PATHS.json)
  }
  ...
  create() {
    ...
    this.add.text(24, 18, 'Water production preview - 80 hero frames + 10 Water Palm FX frames', {
      ...TEXT_STYLE,
      fontSize: '18px',
    })
    this.add.text(24, 46, 'Nearest-neighbor rendering. Matrix is actual 1x; inspector and lower FX lane are 3x.', MUTED_STYLE)
    this.addButton(900, 28, 'Hero Matrix', () => this.showHeroMatrix())
    this.addButton(1065, 28, 'Water Palm', () => this.showBasicFx())
    this.showHeroMatrix()
  }

  registerAnimations() {
    for (const animation of [...WATER_HERO_ANIMATIONS, ...WATER_FX_ANIMATIONS]) {
      const atlas = ['release', 'impact', 'dissipation'].includes(animation.state)
        ? WATER_FX_ATLAS
        : WATER_HERO_ATLAS
      this.anims.create({
        key: animation.key,
  ...
  showHeroMatrix() {
    this.clearView()
    this.track(this.add.text(24, 78, 'Hero Matrix - click any cell to update the 3x inspector', TEXT_STYLE))
    const layout = buildWaterHeroMatrix()
    ...
    for (const item of layout) {
      ...
      const sprite = this.track(this.add.sprite(item.x, item.baselineY, WATER_HERO_ATLAS, item.frames[0])
        .setOrigin(0.5, 56 / 64)
        .setInteractive({ useHandCursor: true }))
  ...
  showInspector(animation) {
    ...
    mark(this.add.text(1160, 138, '3x INSPECTOR', MUTED_STYLE).setOrigin(0.5))
    mark(this.add.line(1068, 410, 1252, 410, 0x6fa0d8, 0.8))
    const sprite = mark(this.add.sprite(1160, 410, WATER_HERO_ATLAS, animation.frames[0])
      .setOrigin(0.5, 56 / 64)
      .setScale(3))
  ...
  showBasicFx() {
    this.clearView()
    const sequence = buildWaterBasicDemo('right')
    this.track(this.add.text(24, 78, `Basic sequence: ${sequence.map(step => step.phase).join(' -> ')}`, TEXT_STYLE))
    this.track(this.add.text(24, 106, 'Water Palm remains close to the striking hand; impact and fade keep one terminal center.', MUTED_STYLE))
    this.startBasicLane({ y: 245, scale: 1, label: 'ACTUAL 1x GAMEPLAY SCALE' })
    this.startBasicLane({ y: 555, scale: 3, label: '3x PIXEL / IDENTITY INSPECTION' })
  }
  ...
  startBasicLane({ y, scale, label }) {
    ...
    const hero = this.track(this.add.sprite(300, y, WATER_HERO_ATLAS, 'attack_right_00.png')
      .setOrigin(0.5, 56 / 64)
      .setScale(scale))
    const fx = this.track(this.add.sprite(390 + 32 * scale, y - 28 * scale, WATER_FX_ATLAS, 'flight_00.png')
      .setScale(scale)
      .setVisible(false))
    const run = () => {
      if (token !== this.viewToken || !hero.active) return
      hero.play('chibi_water_attack_right')
      this.time.delayedCall(110, () => {
        if (token !== this.viewToken || !fx.active) return
        fx.setVisible(true).play('water_basic_fx_release')
        fx.once('animationcomplete', () => {
          fx.play('water_basic_fx_impact')
          fx.once('animationcomplete', () => {
            fx.play('water_basic_fx_dissipation')
            fx.once('animationcomplete', () => {
              fx.setVisible(false)
              this.viewTimers.push(this.time.delayedCall(650, run))
            })
          })
        })
      })
